...
from gsv_metadata_tracker.analysis import is_google_copyright
...
def _age_years(capture_date: Optional[pd.Timestamp], run_ts: pd.Timestamp) -> Optional[float]:
    ...
    if len(panos) > 0:
        ...
        if len(joined) > 0:
            joined = joined.sort_values(
                ["_dist", "capture_date"], ascending=[True, False]
            )
...
def summarize_coverage(covered_edges: gpd.GeoDataFrame) -> Dict[str, Any]:
    ...
    def _block(group: pd.DataFrame) -> Dict[str, Any]:
        ...
        ages = pd.to_numeric(
            group.loc[covered, "nearest_pano_age_years"], errors="coerce"
        ).dropna()
        return {
            "segments": segments,
            "covered": num_covered,
            ...
            "coverage_pct_by_count": round(100.0 * num_covered / segments, 1)
            if segments
            else 0.0,
            "coverage_pct_by_length": round(100.0 * length_km_covered / length_km, 1)
            if length_km
            else 0.0,
            "median_covered_age_years": round(float(ages.median()), 2)
            if len(ages)
            else None,
        }
    ...
    by_type = {
        bucket: _block(group)
        for bucket, group in covered_edges.groupby("highway_bucket")
    }
