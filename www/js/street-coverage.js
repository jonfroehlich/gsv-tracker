 ...
 * gsv_street_analyzer.analyze) and draws each OSM street segment on the map
 * colored by coverage: segments with a nearby pano use the provider age scale
 * (matching the pano dots), segments without are drawn gray/dashed. Also
 * renders a "coverage by street type" panel — a headline plus a stacked
 ...
 *
 * Relies on globals from gsv-utils.js: GSV_DATA_BASE_URL, PROVIDERS, getColor,
 * fetchGzippedJson. Loaded before city.js, which calls renderStreetCoverage().
 *
...
function streetsUrlForDataFile(dataFile) {
  return GSV_DATA_BASE_URL + dataFile.replace(/\.csv\.gz$/, "_streets.json.gz");
}
